function acquireTriggeredOnSongAdvDetectWithRandomSnippets(exper)
...
windowLength = round((actInSampleRate * .6) / (windowSize-windowOverlap)); %song average .9 seconds in length
...
while(true)
    ...
    if(~daq_isRecording(exper.audioCh))
        ...
        %getting too long.
        if( (max(songDet) > threshold) & (sampNum - songStartSampNum < 500000))
            stopSamp = sampNum + length(time) - 1;
        end
            ...
            if(toc < .9)
                ...
                elseif(char == 'y')
                    if(~bPaused)
                        bPaused = true;
                        disp('paused.');
                        oldThres = powerThreshold;
                        powerThreshold = 300000;
                    else
                        bPaused = false;
                        disp('restarted.');
                        powerThreshold = oldThres;
            ...
            if(toc < .9)
                ...
                if( currnum~=0 & (plotnum ~= currnum | plotchan~= currchan))
                    ...
                    if(bDisplay)
                         ...
                         if(length(audio)>0)
                             %plot(timeAxis,audio);
                             displaySpecgramQuick(audio, exper.desiredInSampRate, [0,8000], [-27,15], 0, 1)
