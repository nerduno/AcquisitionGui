...
par.max_clus = 3;                    % maximum number of clusters allowed
...
par.w_pre=15; %20                       % number of pre-event data points stored
par.w_post=20; %44                      % number of post-event data points stored
ref = 0.8;                           % minimum refractory period (in ms)
par.ref = floor(ref *par.sr/1000);   % conversion to datapoints
par.stdmin = 10;   %4                   % minimum threshold for detection
par.stdmax = 120;   %50                  % maximum threshold for detection
...
par.template_sdnum =4;             % max radius of cluster in std devs.
...
par.max_spikes = 2000;               % max. # of spikes to be plotted
