%main file for run and analysis of simulation models
%
%generates an artificial spiketrain and plots it
%runs spike detection, sorting, analysis,projection test on it.
%
...
%thresholdMethod: 1 (normal), 2 ( chi2 statistical test method)       
%upsample: 0 = use 64 datapoints, 1 = use 256 datapoints. thresholdMethod=2 only
%works with upsample=no
...
function [perf,nrAssigned,assigned] = plotGeneratedSpiketrain(basepath, simNr, levelNr, thresholdMethod, upsample, extractionThreshold, export)
...
switch( simNr )
    case 1
        ...
        load([basepath 'simulation1.mat']);
        load([basepath 'simulation1Params.mat']);        
    case 2
        %sim2
        realWaveformsInd=([ 49 36 81 ]);
        load([basepath 'simulation2.mat']);
        load([basepath 'simulation2Params.mat']);        
    ...
    case 3
        ...
        load([basepath 'simulation3.mat']);
        load([basepath 'simulation3Params.mat']);        
        
        if thresholdMethod==1
            reorderN1=[1 2 3 4 5;3 4 5 2 1];
        end
        
        if thresholdMethod==2
            reorderN1=[1 2 3 4 5;4 3 5 2 1];
            reorderN2=[1 2 3 4 5;4 3 5 1 2]
            reorderN3=[1     2     3     4     5; 6     3     5     2     1];
            reorderN4=[1 2 3 4 5;7 3 4 1 2];
        end
end

nrNeurons=length(realWaveformsInd);
noiseStds=[0.05 0.10 0.15 0.20];

%following line only if new spiketrain should be generated
...
%-- save('simulation3.mat', 'spiketrains', 'realWaveformsInd', 'allMeans', 'spiketimes', 'scalingFactorSpikes', 'noiseStds','nrSamples');
%--
...
%waveformsOrig=waveformsOrigAll{levelNr};

%save('spiketrainStamps.mat','spiketimes');

%--- detect spikes
%extractionThreshold=4.0;

%scale arbitrarily - should not change any of the results (control)
%spiketrainDown=spiketrainDown*100;
addNoise=0;
upscale=1;
...
[filteredSignal, rawTraceSpikes,spikeWaveforms, spikeTimestamps, runStd2,upperlim,noiseTraces] = detectArtificialSpikes( spiketrainDown + randn(1,length(spiketrainDown))*addNoise*noiseStd*upscale/10, extractionThreshold ) ;

%estimate std from raw signal
stdEstimate=std(filteredSignal);

spikeWaveformsUnshifted=spikeWaveforms;
spikeWaveforms = realigneSpikes(spikeWaveforms,spikeTimestamps,3,stdEstimate);  %3==type is negative, do not remove any if too much shift
...
if upsample
    ...
    spikeWaveformsUp = realigneSpikes(spikeWaveformsUp,spikeTimestamps,3,stdEstimate);  %3==type is negative, do not remove any if too much shift
...
hold off
title(['Sim1, Noise level ' num2str(levelNr) ' , Filtered 300-3000Hz']);
xlim([1 tillInd-fromInd]);
ylim([-1.1 1.1]);
