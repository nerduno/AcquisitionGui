...
%
function handles = GUISortOnline( hObject, handles, mode, thresholdMethod  )
...
cluNoise=nrAssigned(find(nrAssigned(:,2)<=150),1);
for i=1:length(cluNoise)
    assigned(find(assigned==cluNoise(i)))=9999;
end

cluUse=nrAssigned(find(nrAssigned(:,2)>150),1);
neg='';
